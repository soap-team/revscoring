...
def _process_proportion_of_qid_added(parent_entity, revision_entity):
    parent_entity_doc = parent_entity.toJSON() if parent_entity is not None else {}
    re_qid = re.compile(r'Q\d{1,8}')
    revision_entity_qids = len(re.findall(re_qid, str(revision_entity.toJSON())))
    parent_entity_qids = len(re.findall(re_qid, str(parent_entity_doc)))
...
def _process_proportion_of_language_added(parent_entity, revision_entity):
    ...
    revision_entity_res = len(re.findall(LANGUAGE_RE,
                                       str(revision_entity.toJSON())))
    parent_entity_res = len(re.findall(LANGUAGE_RE, str(parent_entity_doc)))
...
def _process_proportion_of_links_added(parent_entity, revision_entity):
    ...
    revision_entity_res = len(re.findall(re_qid, str(revision_entity.toJSON())))
    parent_entity_res = len(re.findall(re_qid, str(parent_entity_doc)))
    return float(revision_entity_res - parent_entity_res) / \
        float(revision_entity_res + 1)
